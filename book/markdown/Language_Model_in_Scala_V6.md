...
![Three training examples generated by sliding a context window of size 3 across the token sequence. Each example pairs the highlighted context with the next token as its target.](images/fig_sliding_window.png)
...
![Vectors at a glance. A vector is an arrow with a length and a direction (left). Adding two vectors traces a path head-to-tail (middle). The dot product measures how much two vectors agree (right).](images/fig_vectors.png)
...
![Matrix-vector multiplication. Each cell of the output is computed by dot-producting one row of the matrix with the entire input vector. A (2 × 3) matrix turns a 3-dim input into a 2-dim output.](images/fig_matrix_transform.png)
...
![Word embeddings projected to two dimensions. Words with similar meanings end up close together in the embedding space — animals near other animals, cities near other cities — even though no one programmed these categories in.](images/fig_embeddings.png)
...
![The hidden layer as a network of detectors. Every input value connects to every hidden node through a learned weight; training tunes those weights so each hidden node becomes a detector for some useful pattern in the context.](images/fig_hidden_layer.png)
...
![Softmax in action. Raw logits (left) — which can be any real numbers — become a valid probability distribution (right) after exponentiating and normalizing.](images/fig_softmax.png)
...
![The complete forward pass architecture of the language model: context tokens flow through embedding lookup, then a hidden layer with tanh activation, then a linear output layer, then softmax to produce a probability distribution over the vocabulary.](images/fig_forward_pass_architecture.png)
...
![The forward pass: from token IDs to a probability distribution over the vocabulary.](images/fig_forward_pass.png)
...
![Gradient descent as walking downhill in fog. You can't see the whole landscape, but you can feel which way the slope is going. Each step takes you a little closer to the bottom.](images/fig_foggy_valley.png)
...
![Forward and backward through the same path. Black arrows carry values left-to-right during the forward pass; red dashed arrows carry gradients right-to-left during backpropagation. Each layer has both directions running over it.](images/fig_backprop.png)
...
![Training loss over 100 steps on a small corpus. Loss drops quickly at first as the model learns gross structure, then more slowly as it refines.](images/fig_loss_curve.png)
